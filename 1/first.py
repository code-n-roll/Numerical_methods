...
def thomas_algorithm(N, a=a, b=b, A=A, B=B):
    ...
    r = (4.0 - 2.0 * (2.0 - q(a + h) * h ** 2) / (2.0 + p(a + h) * h)) * alpha[
        1]
    z = 2.0 * alpha[0] * h - 3.0 * alpha[1] - alpha[1] * (
        p(a + h) * h - 2.0) / (2.0 + p(a + h) * h)
    kap.append((-r / z))
    nu.append(((2.0 * h * A + 2.0 * alpha[1] * f(a + h) * (h ** 2) / (
        2.0 + p(a + h) * h)) / z))

    r = (-4.0 + 2.0 * (2.0 - q(b - h) * (h ** 2)) / (2.0 - p(b - h) * h)) * \
        beta[1]
    z = 2.0 * beta[0] * h + 3.0 * beta[1] - beta[1] * (p(b - h) * h + 2.0) / (
        2.0 - p(b - h) * h)
    kap.append((-r / z))
    nu.append(((2.0 * h * B - 2.0 * beta[1] * f(b - h) * (h ** 2) / (
        2.0 - p(b - h) * h)) / z))

    u.append(kap[0])
    v.append(nu[0])

    grid = ((j[0], a + float(j[1]) * (b - a) / N) for j in
            enumerate(range(0, N)))
